...
async function buildThemeAssets() {
  ...
  for (const scriptName in themeScripts) {
    rootPackageJson.scripts[
      `theme:${scriptName}`
    ] = `cd ./wp-content/themes/${themeName} && npm run ${scriptName}`;
  }
  ...
  console.log(
    chalk.green(`Theme scripts copied to the root package.json successfully!`)
  );
}
...
async function main() {
  ...
  const { installComposer } = await inquirer.prompt([
    {
      ...
      message: chalk.black.bgWhite(
        "Do you want to install composer dependencies?"
      ),
      default: true,
    },
  ]);
  ...
  if (installComposer) {
    ...
    try {
      ...
      const themeName = fs
        ...
        .split("\n")
        .find((line) => line.startsWith("THEME_NAME"))
        .split("=")[1];
      if (!themeName) {
      ...
      console.log("Theme dependencies installed successfully!");
    } catch (error) {
      console.error("Error installing theme dependencies:", error);
    }
  }
  ...
  if (installPlugins) {
    ...
    function getContainerName() {
      ...
      const containerName =
        readEnvVariable(envFilePath, "PROJECT_NAME") + "_wordpress";
      if (!containerName) {
        console.error(
          "Error: PROJECT_NAME environment variable not found in .env file."
        );
    ...
    console.log(
      chalk.green("Development plugins installed, see logs above for details!")
    );
  }
